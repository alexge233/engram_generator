...
register_atom(Atom(
    ...
    example="f(x)=x^2, f'(x)=2x. x_0=5, lr=0.1: x_1 = 5 - 0.1*10 = 4. x_2 = 4 - 0.1*8 = 3.2",
    tier=4,
    domain="optimization",
...
register_atom(Atom(
    ...
    example="lr_0=0.1, decay_rate=0.96, epoch=10: lr = 0.1*0.96^10 = 0.1*0.6648 = 0.0665",
    tier=4,
    domain="optimization",
...
register_atom(Atom(
    ...
    example="Input 32x32, kernel 5x5, stride 1, padding 0: output = (32-5+2*0)/1 + 1 = 28x28",
    tier=4,
    domain="neural_networks",
...
register_atom(Atom(
    ...
    example="V(s) = max_a[R(s,a) + gamma*V(s')]. State s1: actions left(R=1,s2), right(R=5,s3). gamma=0.9, V(s2)=3, V(s3)=2. V(s1) = max(1+0.9*3, 5+0.9*2) = max(3.7, 6.8) = 6.8",
    tier=6,
...
register_atom(Atom(
    ...
    example="Q(s1,left)=5, R=1, max Q(s2,.)=8, gamma=0.9, alpha=0.1: Q(s1,left) += 0.1*(1+0.9*8-5) = 5 + 0.1*3.2 = 5.32",
    tier=6,
    domain="reinforcement_learning",
...
register_atom(Atom(
    ...
    example="REINFORCE: grad J = E[G_t * grad log pi(a|s)]. Episode return G=10, pi(a|s)=0.3: grad log pi = (1/0.3)*grad_pi. Weight update = lr * 10 * grad log pi",
    tier=6,
    domain="reinforcement_learning",
...
register_atom(Atom(
    ...
    example="State s, action a: R(s,a)=10, transitions to s' with P=0.8 and s'' with P=0.2. Expected reward = 0.8*10 + 0.2*10 = 10",
    tier=5,
    domain="reinforcement_learning",
...
register_atom(Atom(
    ...
    example="Rewards [10, 5, 3], gamma=0.9: G = 10 + 0.9*5 + 0.81*3 = 10 + 4.5 + 2.43 = 16.93",
    tier=5,
    domain="reinforcement_learning",
    source="Wikipedia, 'Markov decision process'",
    source_url="https://en.wikipedia.org/wiki/Markov_decision_process",
...
register_atom(Atom(
    ...
    example="MSE = Bias^2 + Variance + Noise. Model with Bias^2=4, Var=1, Noise=0.5: MSE = 4+1+0.5 = 5.5. Reducing bias by increasing complexity may increase variance",
    tier=5,
    domain="model_evaluation",
